...
public class BST<T extends Comparable<? super T>> {
    ...
    public String ancestors(T key) {
        String ancestors = "";

        return this.ancestors(this.root, key, ancestors).trim();
    }

    public String ancestors(BSTNode<T> root, T key, String result) {
        // write your code here
        // check if current node data equals to key
        if(root.el == key)
            return result;
        // else if the node data is larger than key, go to the left
        if(root.el.compareTo(key) > 0) {
            // add the current ancestor to result
            result += root.el + " ";
